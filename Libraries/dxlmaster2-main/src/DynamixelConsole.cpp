...
DynamixelConsole::DynamixelConsole(Stream &aConsole , uint8_t aVersion) 
    : mConsole(aConsole), mVer(aVersion)
{
...
void DynamixelConsole::loop()
{
    ...
    {
        if (c >= 32 && c <= 126 && (mLinePtr - &(mLineBuffer[0])) < (int)sLineBufferSize)
        {
            mConsole.write(c);
            *mLinePtr = c;
            ++mLinePtr;
